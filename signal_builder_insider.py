...
# Filtres de base : période, prix valide, shares > 0, open-market P/S uniquement
...
# Dans signal_builder, juste après le filtre classification
df_insider = df_insider[
...
def classify_owners(owner_months: pd.DataFrame) -> pd.DataFrame:
    ...
    for owner_cik, group in owner_months.groupby('owner_cik'):
        ...
        for year in years:
            ...
            # Moins de 3 ans d'historique → inclassifiable
            if len(prev_years) < 3:
                results.append({
                    'owner_cik'      : owner_cik,
                    'year'           : year,
            ...
            # 3 années précédentes les plus récentes
            prev_3 = prev_years[-3:]
 
            # Intersection des mois sur ces 3 années
...
# Les owners sans historique suffisant → insufficient_history
...
# Month sur filing_date (point-in-time)
...
# Agrégation ticker × month × signal_key
agg = (
    df_insider
